...
function openIntake(type,project=null){
  $('intakeTitle').textContent=type==='logo'?'Logo creation intake':'Web design intake';
  $('intakeForm').innerHTML=type==='logo'?logoFields:webFields;
  $('intakeForm').dataset.type=type;
  ...
  f.elements.business_name.value=profile?.company_name||'';
...
async function submitIntake(e){
  ...
  if(projectId){
    if(type==='web'){
      const features=String(obj.required_features||'').split(',').map(x=>x.trim()).filter(Boolean);
      obj.required_features=features;
    }
  ...
  if(type==='web'){
    obj.required_features=String(obj.required_features||'').split(',').map(x=>x.trim()).filter(Boolean);
    const {error}=await db.from('web_intakes').insert(obj);
    if(error)return toast(error.message);
  }else{
    const {error}=await db.from('logo_intakes').insert(obj);
    if(error)return toast(error.message);
  }
  toast('Design intake submitted.');
  close();

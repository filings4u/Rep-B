...
async function openDrawer(id){
  ...
  $('drawerContent').innerHTML=`
    ...
    <article class="invoice-preview">
      <div class="invoice-preview-top"><div><img src="images/logo.png" alt="filings4u"><small>A Subsidiary of Roseland Companies, LLC</small></div><div><h3>${esc(inv.invoice_number||'Invoice')}</h3><small>Due ${date(inv.due_date)}</small></div></div>
...
/* filings4u dedicated invoice management route */
document.getElementById('newInvoiceManagementTop')?.addEventListener('click',e=>{
  e.preventDefault(); window.location.href='admin-invoice-management.html';
});
document.getElementById('newInvoiceManagementButton')?.addEventListener('click',e=>{
  e.preventDefault(); window.location.href='admin-invoice-management.html';
});

// ============================================================================ // 
// infrastructure-part-a.js: SECURE CORE DB BOOTSTRAPPER                       // 
// ============================================================================ // 
(function() {
  "use strict";

  // FIXED: Corrected a broken quote escaping character replacement syntax match
  ...
  // 🚀 FIXED: Explicit execution loop added. Previously, this process was declared but never fired!
  bootstrapGlobalSupabaseSystem();
})();

// ============================================================================ // 
// infrastructure-part-b.js: AUDITING & INACTIVITY TIMEOUT GUARDS              // 
// ============================================================================ // 
(function() {
  "use strict";

  // GLOBAL AUTH HOOKS & TIMEOUT GUARD
  ...
  // INACTIVITY BARRIER GATE CONTROLLER (ADMIN vs CUSTOMER)
  ...
  async function purgeAdminSession() {
    console.warn("Security Alert: Admin session idle for 15 minutes.");
    if (window.supabaseClient?.auth) { try { await window.supabaseClient.auth.signOut(); } catch(e){} }
  ...
  async function purgeClientSession() {
    ...
    if (window.supabaseInstance?.auth) { try { await window.supabaseInstance.auth.signOut(); } catch(e){} }
  ...
  function startInteractionTracking() {
    ...
    const isClient = path.includes('client-') || path.includes('/client');

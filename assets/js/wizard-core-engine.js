...
document.addEventListener("DOMContentLoaded", () => { 
    ...
    // CRITICAL RECOVERY: Injected the click event button back into the Step 1 layout template string
    wrapper.innerHTML = ` 
        ...
        <button type="button" onclick="transitionToUpsellStep()" style="background:#0a1f44; color:white; border:none; width:100%; padding:14px; border-radius:6px; font-weight:700; font-size:1rem; cursor:pointer; margin-top: 25px;">Continue to Upsells ➔</button>
    `; 
...
async function transitionToUpsellStep() { 
    ...
    // Mirror entry fields straight into local cache arrays using BOTH naming standards to prevent summary drops
    ...
    // Legacy mapping keys back-sync configurations
    sessionStorage.setItem('globalCachedUserEmail', globalCachedUserEmail);
    sessionStorage.setItem('globalCachedCompanyName', globalCachedCompanyName);
...
function transitionToSummaryStep() { 
    ...
    if (step3) { 
        ...
        // Execute line-item totals calculations and compile legal contract strings natively
        if (typeof calculateRealTimeSummaryMatrix === "function") {
